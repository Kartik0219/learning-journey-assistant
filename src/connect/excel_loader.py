...
CURATED_RESOURCES_CSV = Path(__file__).resolve().parents[2] / "data" / "dataset" / "curated_resources.csv"


def load_topic_materials() -> pd.DataFrame:
    """The workbook has no subject topic-material passages at all (F9's
    grounded-citation source). Returning nothing left Study plan, Quizzes
    and Resources empty for every one of the 150 students, so this loads
    the team's curated open-resource list (`data/dataset/curated_resources.csv`)
    instead. Each row is a well-known free resource (official docs, MIT OCW,
    OWASP...) tagged with the SILO it supports, plus a short passage saying
    what it covers and how to use it. Rows are marked provenance="curated"
    and the UI labels them as such: they are the team's picks, not La Trobe
    subject material. Nothing is generated by a model. If the CSV is absent
    the honest empty frame comes back and src.estimate.mastery's "No topic
    material is available yet" fallback still applies."""
    if not CURATED_RESOURCES_CSV.exists():
        return pd.DataFrame(columns=_MATERIAL_COLUMNS)
    df = pd.read_csv(CURATED_RESOURCES_CSV, dtype=str).fillna("")
    if "provenance" not in df.columns:
        df["provenance"] = "curated"
    df["provenance"] = df["provenance"].replace("", "curated")
