  ...
  return <main className="dashboard" id="dashboard">
    ...
    <div className="content-grid">
      ...
      <div className="right-column">
        <section className="panel"><div className="panel-heading"><h2>Next steps for {selectedSiloId}</h2><p>{selectedOutcome.name} · {selectedOutcome.statusLabel} ({selectedOutcome.masteryPercentage}%)</p></div><ol className="steps">{(selectedOutcome.nextSteps ?? []).map((step, index) => <li key={step.text}><span>{index + 1}</span><p>{step.text}{step.emphasis && <strong>{step.emphasis}</strong>}{step.suffix}</p></li>)}</ol></section>
        <section className="panel"><div className="panel-heading"><h2>Recommended resources for {selectedSiloId}</h2><p>{selectedOutcome.name}</p></div><ul className="resources">{(selectedOutcome.recommendedResources ?? []).map((resource) => <li key={resource}><span /><a href="#resources">{resource}</a></li>)}</ul></section>
      </div>
    </div>
